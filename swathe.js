/*!
 * Swathe.js — fabric physics for Three.js
 * https://github.com/McMuffinLeq/swathejs
 * MIT License
 *
...
(function (global, factory) {
  ...
  function resolveThree(explicit) {
    ...
    if (!THREE) {
      throw new Error('Swathe.js: THREE.js not found. Load Three.js before Swathe.js, or pass { THREE } in options.');
    }
    return THREE;
  }
   ...
   * use Swathe.cloth(scene, options) instead, which also builds and
  ...
  Cloth.prototype._resolveColliders = function (colliders) {
    ...
    for (var p = 0; p < n; p++) {
      ...
      for (var cIdx = 0; cIdx < colliders.length; cIdx++) {
        ...
        if (Math.abs(lx) >= half.x + skin || Math.abs(ly) >= half.y + skin || Math.abs(lz) >= half.z + skin) continue;
        ...
        if (onTop) {
          ...
          if (pastEdge) continue; // no support out here — let gravity take it
        }

        var nx = 0, ny = 0, nz = 0;
        if (ox < oy && ox < oz) {
          nx = lx < 0 ? -1 : 1;
          pos[pxI] = cx + nx * (half.x + skin);
        } else if (oy < oz) {
          ny = ly < 0 ? -1 : 1;
          pos[pyI] = cy + ny * (half.y + skin);
        } else {
          nz = lz < 0 ? -1 : 1;
          pos[pzI] = cz + nz * (half.z + skin);
        }
        ...
        px = pos[pxI]; py = pos[pyI]; pz = pos[pzI];
      }
    }
  };
